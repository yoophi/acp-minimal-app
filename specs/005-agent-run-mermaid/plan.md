...
Agentic Workbench의 agent-run streaming Markdown 렌더러에서 `mermaid` fenced code block을 Mermaid 다이어그램으로 표시한다. 기존 agent-run Markdown/code block 표시 흐름은 유지하고, Mermaid 렌더링은 블록 단위로 실패 격리한다. 이미 workspace package로 존재하는 `@yoophi/markdown-annotation-react`의 Mermaid 렌더링 컴포넌트와 `@yoophi/markdown-annotation-core`의 감지 helper를 재사용해 중복 renderer를 만들지 않되, 적용 범위는 `apps/agentic-workbench/src/features/agent-run`의 agent-run 출력 렌더러로 제한한다.
