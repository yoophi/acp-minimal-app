...
**Independent Test**: Mermaid 코드 블록이 streaming 중에 점진적으로 완성되는 응답과 넓은 다이어그램이 포함된 응답을 표시하고, partial 상태와 완성 상태 모두에서 agent-run 레이아웃이 유지되는지 확인한다.
...
- Contained overflow may use scrolling or equivalent navigation; fitting every diagram fully into the visible panel without scrolling is not required.
