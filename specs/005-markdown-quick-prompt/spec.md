...
**Input**: User description: "GitHub issue #115: Markdown annotator에 quick prompt 기능 추가. 선택 영역, Markdown 블럭, 또는 전체 문서 내용을 바탕으로 번개 아이콘 quick prompt/quick annotate를 실행하고, 해당 컨텍스트가 첨부된 프롬프트를 작성/수정한 뒤 agent-run에게 바로 전달할 수 있게 한다. 영역을 선택한 후 표시되는 툴바에도 quick annotate 버튼이 표시되어야 한다. 첨부 범위 식별, disabled/empty 상태, 기존 annotation 선택/블럭 식별 로직과의 충돌 방지, 긴 문서 첨부 정책, agent 응답 연결 방식, tooltip/keyboard 접근성이 필요하다."

## User Scenarios & Testing *(mandatory)*

### User Story 1 - 선택 영역을 agent 질문으로 전환 (Priority: P1)

Markdown 문서를 검토하는 사용자는 문서 안의 특정 문장이나 구간을 선택한 뒤 선택 영역 위에 표시되는 툴바의 번개 아이콘으로 quick prompt 작성 UI를 열고, 선택한 텍스트가 첨부된 상태에서 agent에게 질문이나 작업 요청을 보낼 수 있다.

**Why this priority**: 현재 사용자가 수동 복사/붙여넣기로 처리하는 가장 빈번한 문서 검토 흐름을 끊김 없이 대체하는 핵심 가치이다.

**Independent Test**: 문서 일부 텍스트를 선택한 뒤 표시되는 선택 영역 툴바에서 quick annotate 버튼을 실행하고, 선택 범위가 첨부 컨텍스트로 표시되며 사용자가 작성한 프롬프트와 함께 agent-run에 전달되는지 확인한다.

**Acceptance Scenarios**:

1. **Given** Markdown 문서에서 텍스트 일부가 선택되어 있다, **When** 선택 영역 툴바가 표시된다, **Then** 툴바에는 quick prompt/quick annotate 번개 아이콘 버튼이 표시된다.
2. **Given** 선택 영역 툴바가 표시되어 있다, **When** 사용자가 quick prompt/quick annotate 번개 아이콘을 실행한다, **Then** 사용자 입력을 받을 수 있는 prompt 작성 다이얼로그가 열리고 첨부 컨텍스트는 "선택 영역"으로 식별된다.
3. **Given** 선택 영역 quick prompt 작성 UI가 열려 있다, **When** 사용자가 프롬프트 내용을 작성하거나 수정한다, **Then** 첨부 컨텍스트는 유지되고 사용자의 프롬프트 본문만 편집된다.
4. **Given** 선택 영역과 프롬프트 본문이 준비되어 있다, **When** 사용자가 전송한다, **Then** 선택 텍스트가 포함된 요청이 활성 agent-run에 전달된다.
...
1. **Given** 사용자가 Markdown 문서의 특정 블럭을 보고 있다, **When** 해당 블럭의 quick prompt/quick annotate 번개 아이콘을 실행한다, **Then** 사용자 입력을 받을 수 있는 prompt 작성 다이얼로그가 열리고 첨부 컨텍스트는 "Markdown 블럭"과 블럭 위치/요약으로 식별된다.
2. **Given** 블럭에 Markdown 원문과 렌더링된 표시가 모두 존재한다, **When** quick prompt 컨텍스트가 구성된다, **Then** agent가 작업할 수 있는 충분한 블럭 원문이 첨부된다.
3. **Given** 사용자가 블럭 단위 prompt를 작성했다, **When** 사용자가 다이얼로그에서 전송을 누른다, **Then** 사용자가 입력한 프롬프트와 블럭 컨텍스트가 활성 agent-run으로 전송된다.
...
3. **Given** 문서 단위 prompt가 작성되어 있다, **When** 사용자가 다이얼로그에서 전송을 누른다, **Then** 요청은 사용자가 작성한 프롬프트와 문서 컨텍스트를 함께 포함해 활성 agent-run으로 전송된다.
...
- **FR-001a**: The lightning icon entry point MAY be labeled or referred to as quick annotate in UI copy, but it MUST open the same quick prompt flow.
- **FR-001b**: When a user selects a document region and the selection toolbar is shown, the toolbar MUST include a quick annotate lightning icon button.
...
- **FR-006a**: Clicking a quick annotate/quick prompt button MUST open a dialog that accepts user-entered prompt text before any send action is available.
- **FR-007**: The system MUST send the user's prompt text together with the attached context to the active agent-run selected for the current workspace/session.
- **FR-007a**: The system MUST send the actual prompt text entered in the dialog; it MUST NOT replace the user's text with a default prompt when the user has provided input.
...
- **Agent Target**: The active agent-run destination that can receive the prompt request for the current workspace/session; has availability and sendability state visible to the user.
...
- **SC-001a**: In representative verification, the selection toolbar displays a quick annotate button after a non-empty region selection in 100% of tested valid selections.
...
- **SC-006a**: In representative verification, clicking a quick annotate button opens a user-input dialog and sends the entered prompt text to agent-run in 100% of valid send cases.
...
- "바로 agent에게 전달" means sending to the active agent-run associated with the current workspace/session after the user reviews and submits the prompt draft.
